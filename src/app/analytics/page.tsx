      ...
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Compare metrics, analyze bundle size and pricing benchmarks, and view your event
        ...
        <div className="md:col-span-2 space-y-6">
          <Card className="border shadow-sm">
            ...
            <CardContent className="divide-y divide-border">
              {/* Amplitude */}
              {providers.map((p) => {
                const isAmplitude = p.id === 'amplitude';
                const isToggled = p.enabled;
                const isInitialized = p.initialized;
                  ...
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        {isToggled ? (
                          isAmplitude && !isAmplitudeConfigured ? (
                            <Badge
                              variant="outline"
                      ...
                      {isAmplitude && (
                        <p className="text-xs text-muted-foreground font-mono">
                          ...
                          {isAmplitudeConfigured
                            ? '✅ Set via env'
                            : '⚠️ Missing (NEXT_PUBLIC_AMPLITUDE_API_KEY)'}
                        </p>
                      )}
                    </div>
          ...
          <Card className="border shadow-sm">
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Recent tracking signals processed. Events logged to Amplitude, PostHog, or
                  Mixpanel show in their respective badges.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent>
              {logs.length === 0 ? (
                ...
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                  {logs.map((log) => {
                    ...
                    return (
                      // biome-ignore lint/a11y/useSemanticElements: block container is cleaner than button for rich nested markup
                      <div
                        key={log._id}
                      ...
                      >
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            {/* Badges for dispatched destinations */}
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 text-xs">
                              <div className="bg-muted/50 p-3 rounded border space-y-2">
                                ...
                                <div className="font-mono space-y-1 text-foreground">
                                  ...
                                  <div>
                                    <strong className="text-muted-foreground">To (Path):</strong>{' '}
                                    <span className="break-all">
                                      {log.properties.$path || 'N/A'}
                                    </span>
                                  </div>
                                  {log.properties.source_url && (
                                    <div>
                                      <strong className="text-muted-foreground">
                                        Source page:
                                      </strong>{' '}
                                      <span className="break-all">{log.properties.source_url}</span>
                                    </div>
                                  )}
                                  {log.properties.destination_url && (
                                    <div>
                                      <strong className="text-muted-foreground">
                                        Destination page:
                                      </strong>{' '}
                              ...
                              <div className="bg-muted/50 p-3 rounded border space-y-2">
                                ...
                                <div className="font-mono space-y-1 text-foreground">
                                  ...
                                  {log.properties.title && (
                                    <div>
                                      <strong className="text-muted-foreground">Post Title:</strong>{' '}
                                      {log.properties.title}
                                    </div>
                                  )}
                                  {log.properties.author && (
                                    <div>
                                      <strong className="text-muted-foreground">Author:</strong>{' '}
                                      {log.properties.author}
                                    </div>
                                  )}
                                  {log.properties.tags && log.properties.tags.length > 0 && (
                                    <div>
                                      <strong className="text-muted-foreground">Tags:</strong>{' '}
                                      {Array.isArray(log.properties.tags)
                                        ? log.properties.tags.join(', ')
                                        : String(log.properties.tags)}
                                    </div>
                                  )}
                                </div>
                              </div>
          ...
          <Card className="border shadow-sm">
            ...
            <CardContent className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground italic">
                  Note: Local DB is fastest due to server-side optimization and local deployment.
                  PostHog latency is slightly higher due to richer recording payload sizes.
                </p>
              </div>
            </CardContent>
          </Card>
